...
intelligence proposes a constrained Recipe; Pasted validates, previews,
...
Recipe is regenerated by another provider.
...
A Recipe is a provider-generated, versioned execution plan. Each step is either
a validated deterministic tool reference or a semantic instruction executed by
a Connection. Recipes may be pinned or regenerated adaptively. Their execution
character is reported separately as replayable, interpretive, or mixed.

- Pinned means the Recipe does not change without approval; it does not promise
...
- Mixed Recipes use both, allowing AI to compile away work that does not need a
...
Operations form a constrained tool vocabulary for Recipe planners. The normal
...
Pipelines are the current deterministic Recipe representation. They may remain
as an Advanced inspector or be replaced by the versioned Recipe format once the
...
- **Transform** asks what should happen, accepts representative examples, lets
  ...
  proposed output and Recipe before approval.
- **Recipes** lists saved Intents using human descriptions and reports whether
  their current Recipe is replayable, interpretive, or mixed.
