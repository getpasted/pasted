...
| Library Storage (`library_storage.rs`) | GUI `commands/{storage,snapshots}.rs` and CLI Storage and Snapshot commands translate requests, pick destinations, and format output. | `library_storage/{relocation,recovery,factory_reset,snapshots}.rs` own coordinated moves, startup fallback, reset cleanup, scheduling, retention, publication, verification, and activation policy. | `DbSession` coordinates access to the active SQLite library; Snapshot databases and metadata live independently in application data so a moved library can be recovered. | Native file and folder pickers stay asynchronous in GUI commands; filesystem copying, hashing, syncing, and SQLite inspection run outside the Tauri command thread. |
...
Settings, Snapshots, storage, Suggestions, and Transforms have focused adapters. Shared
