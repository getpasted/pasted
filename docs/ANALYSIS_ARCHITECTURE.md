...
- `db/stored_analysis.rs` is a declaration-only facade. Its focused owners are `classifications.rs` for hash-safe semantic matches, `inspections.rs` for durable structural results, `extractions.rs` for current Extractor observations and attempt history, `searchable_text.rs` for derived searchable text, and `ocr.rs` for OCR candidates and backfill lifecycle state. These modules validate current clip identity and own their transactions; they do not reinterpret Analyzer scheduling or participant outcomes.
