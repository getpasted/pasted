...
Structure Inspection, File Format Inspection, Media Metadata, and Smart Actions are immutable built-in participants in 1.0. Structure Inspection is always available because Clip Preview depends on its bounded structural facts. File Format Inspection follows its Functionality switch and persists byte-verified formats. The locally powered Media Metadata Inspector and Smart Actions run only under the interactive policy. Smart Actions also follows the user-facing Transformations feature state. Their definitions and contracts remain inspectable through `pasted inspector`, `pasted suggestion`, and `pasted registry`.
...
- `db/stored_analysis.rs` is a declaration-only facade. Its focused owners are `classifications.rs` for hash-safe semantic matches, `inspections.rs` for durable structural results, `extractions.rs` for current Extractor observations, `attempts.rs` for bounded per-Clip/per-Extractor history and reuse metadata, `types.rs` for shared records, `searchable_text.rs` for derived searchable text, and `ocr.rs` for OCR candidates and backfill lifecycle state. These modules validate current clip identity and own their transactions; they do not reinterpret Analyzer scheduling or participant outcomes.

GUI Analysis commands, CLI Analysis adapters, capture, and background workers
all consume the same participant execution contracts before calling those
persistence owners. Filesystem availability, external file size, and media
metadata remain live observations and must not migrate into stored Analysis.
...
The shipped `inspector:file-format-v1` participant consumes file references and reads at most a bounded prefix from a bounded number of files. It produces persisted `file_formats` from byte signatures rather than filename extensions. Multi-file clips may produce several formats. Rescans can backfill existing clips without changing original content.
...
Extractor definitions use one `recipe-v1` runtime contract across shipped and custom entries. A recipe records accepted representations, direct local command steps, absolute or discovered executables, argv tokens, step artifacts, output capture, resources, time limits, declared post-processing, scheduling state, and revision. The runner never invokes a shell or AI. It clears unneeded environment variables, uses a private workspace, validates placeholders and resources, bounds execution and output, and removes staging afterward. Runtime status resolves every command and resource into visible readiness. Post-processing is a provider-neutral ordered list applied by the shared runner; shipped and custom recipes can use the same operations. The initial `filter_labels_by_confidence` operation filters scored labels before the remaining labels become searchable text, while labels without a confidence score remain eligible.
