...
The shipped `inspector:media-metadata-v1` participant consumes file references only during interactive inspection and produces bounded aggregate `media_metadata`. Its current `ffprobe-cli-v1` engine invokes a discovered ffprobe executable directly with fixed arguments, a five-second total timeout, an eight-file ceiling, private output staging, and a bounded JSON parser. Participant identity and output semantics remain independent of the selected engine so another local implementation can produce the same contract. Media metadata and external paths remain live and are not persisted.
