...
Lock Pasted can be assigned under **Settings → Hotkeys** and defaults to `Option+Shift+L` on macOS or `Alt+Shift+L` on Windows and Linux. Unlock from the lock screen with the passphrase or an enabled system authentication method; shortcuts never bypass authentication.
