...
**Settings → Storage → Full Backup and Restore** creates an exact recovery point: a validated, unencrypted SQLite snapshot of all durable state owned by Pasted, plus saved interface/window state. This includes clips in History and Trash, stored clipboard images, all Bins and ordering, revisions, Activity, settings and hotkeys, App Exclusion rules, Copy Queue state, Transforms, Operations, automations, execution history, OCR state, Extractors, Classifiers, derived Analysis classifications, Content Types, and intelligence connection setup. Store the file on encrypted storage or place it in an encrypted archive when additional protection is needed.
