# Pasted

Pasted is a local, privacy-first clipboard manager for text, images, screenshots, PDFs, files, and the fragments that ordinary clipboards forget.

## Start here

- [Getting Started](Getting-Started)
- [History and Search](History-and-Search)
- [Activity and Insights](Activity-and-Insights)
- [Bins and Organization](Bins-and-Organization)
  - [Smart Bin Rule Contract](Smart-Bin-Rule-Contract)
- [Queue and HUD](Queue-and-Quick-HUD)
- [Transforms and Intelligence](Transforms-and-Intelligence)
- [Files, OCR, and Previews](Files-OCR-and-Previews)
- [Backup, Trash, Revisions, and Recovery](Backup-Trash-Revisions-and-Recovery)
- [Settings and Features](Settings-and-Features)
- [Content Analysis and Content Types](Classification-and-Types)
- [Notifications and Capture Feedback](Notifications-and-Capture-Feedback)
- [CLI Reference](CLI-Reference)

## Trust and support

- [Privacy, Security, and Safety](Privacy-Security-and-Safety)
- [Platform Support](Platform-Support)
- [Troubleshooting and Diagnostics](Troubleshooting-and-Diagnostics)
- [Building, Contributing, and Releasing](Building-Contributing-and-Releasing)

Pasted 1.0 officially supports macOS 13 or newer on Apple Silicon and Intel. Linux is available as an x86_64 AppImage preview. Windows builds are unsigned and experimental.
