...
The built-in Structure Inspector records content-free text counts, image dimensions, file item counts and filename extensions, and the derived clip origin. The File Format Inspector separately reads bounded byte signatures and persists verified formats. Stable results are fingerprinted and persisted; filesystem availability and total file size remain live observations. Clip Preview and `pasted inspector run` consume the same versioned results.
...
Shipped recipes provide guided setup for the first missing executable or model. Setup commands are displayed as copyable, reviewable commands and never run or download a model automatically. Preflight checks the selected executable, resources, and version command without sending clip content. If a recipe still cannot run, a configured Intelligence connection can draft a repair for review; it cannot silently change or enable the recipe.
...
A recipe may declare specific nonzero exit codes as expected no-output results. Use this only when the underlying tool documents that exit as a normal “nothing found” condition. Other nonzero exits remain failures with bounded diagnostics. Expected no-output configuration, output post-processing, and recipe changes are preserved in local authoring history and the editable recipe contract.
