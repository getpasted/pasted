...
Extraction runs every enabled, available Extractor that accepts the clip input, ordered by priority. Successful outputs are deduplicated and combined for search and later classification. Clip Preview shows each successful result separately and keeps no-output or failed attempts in a subdued per-clip activity disclosure. Whisper requires both `whisper-cli` and a configured local GGML model file; no model is downloaded automatically.
