...
Extractor run JSON serializes the shared Analyzer execution result. It includes `targetKind`, `targetRef`, `outcome` (`produced`, `no_output`, or `failed`), `output`, `detectedType`, `matchedDetectorRef`, a structured `failure`, privacy-safe `participants`, and the CLI-specific `appliedClipId`. Failures use stable codes and neutral messages, exit nonzero, and never include input image or clipboard content.

Detector run JSON serializes the corresponding shared Detection application result. It includes `outcome` (`matched`, `no_match`, or `failed`), `matched`, `detectedType`, `matchedDetectorRef`, `failure`, `participants`, and `appliedClipId`. Applied runs execute against the clip inside the database transaction, so the reported result and mutation cannot diverge. Participant summaries contain only stable references, passes, outcomes, and neutral failures—not analyzed text.
