...
`pasted search` uses the same query grammar, exact case-insensitive collection-axis filters, Functionality gates, chronological ordering, and extracted-text index as Search in the app and Quick HUD. `--clip`, `--content`, `--format`, and `--source` combine with helpers in the query. `--json` returns `{ "items", "totalCount", "limit", "offset" }`; each item keeps the stable snake-case Clip fields, and extracted OCR or transcript text is not returned.

`copy` accepts bounded stdin when text is omitted. `list` and `search` provide bounded pagination; both can inspect Trash, while `list` can select a Bin or pinned clips. `search` reproduces Clip Type, Content Type, File Format, and Source views with exact filters. Its structured records expose canonical `content_type`, `content_types`, `file_formats`, and `source` fields. A disabled Functionality axis cannot be used as a search filter. `import sources` reports supported managers and detected locations. `import` reads a source without modification and merges supported text while skipping duplicates. `retention` manages History, Trash, Activity History, and per-clip revision policies. `settings` reads or changes persisted values; app-bound visual or operating-system effects apply when the app observes the setting or next launches. `clear` requires `--yes` and permanently removes unpinned, unprotected clips from History.
...
pasted extractor create [--name <name>] [--description <text>] (--recipe <recipe.json> | --prompt <request>) [--connection <id>] [--priority <number>] [--enabled|--disabled] [--json]
pasted extractor update <ref> --recipe <recipe.json> [options] [--json]
