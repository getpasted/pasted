...
Background OCR, manual GUI extraction, CLI Extractor application, and CLI rescans persist Analyzer reports through the same application service. The shared path records OCR state with identical failure and stale-clip semantics. Derived classification persistence is best-effort: a classification write failure never reinterprets successfully stored OCR text as an extraction failure, and a later analysis run can rebuild the derived metadata.
