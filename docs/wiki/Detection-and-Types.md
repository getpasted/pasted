...
Extractor failures remain distinct from valid no-text results throughout Analysis. Background and applied runs record the stable failure code for retry and diagnostics, while CLI JSON reports `outcome` and a structured `failure` without including image or clipboard content.
