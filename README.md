...
- Assigns global hotkeys directly to durable clips for immediate paste-by-ID access; assigned clips are protected automatically.
...
- Creates configurable automatic Snapshots when clips change, complete validated Full Backups, and portable History and Organization JSON for merging into another library.
...
Enable or disable all system-wide hotkeys under **Settings → Functionality → Hotkeys**. Individual assignments remain available under **Settings → Hotkeys** while the feature is enabled.
...
Search is authoritative across the GUI, HUD, and CLI. Collection-axis filters match partial values case-insensitively, and paginated JSON search output includes `schemaVersion`, `items`, `totalCount`, `limit`, and `offset` without exposing extracted OCR or transcript text. Search pages contain at most 500 items.
