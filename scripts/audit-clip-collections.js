...
assert.match(clipSearch, /startsWith\('source:'\)/, 'Source filtering must use the canonical source: search operator');
assert.doesNotMatch(clipSearch, /startsWith\('app:'\)/, 'The pre-1.0 app: search operator must not remain as an alias');
assert.match(clipSearch, /sources:\s*string\[\]/, 'Parsed search plans must expose canonical source terminology');
assert.doesNotMatch(clipSearch, /apps:\s*string\[\]/, 'Parsed search plans must not expose the removed app terminology');
