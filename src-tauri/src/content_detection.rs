...
pub const DETECTOR_PRESETS: &[DetectorPreset] = &[
    DetectorPreset {
        stable_ref: "color",
        name: "Colors",
        content_type: "color",
...
pub fn validate_detector_input(input: &DetectorInput) -> Result<(), String> {
    ...
    if input.description.chars().count() > 500 {
        return Err("Detector descriptions are limited to 500 characters".into());
    }
    if !(-10_000..=10_000).contains(&input.priority) {
        return Err("Detector priority must be between -10,000 and 10,000".into());
    }
    if input.validator.as_deref().is_some_and(|validator| {
        ...
        return Err("Unknown detector validator".into());
    }
    if input.patterns.is_empty() || input.patterns.len() > 16 {
        return Err("Detectors require 1–16 regex patterns".into());
    }
    for pattern in &input.patterns {
        if pattern.len() > 2_048 {
            return Err("Detector regex patterns are limited to 2,048 characters".into());
        }
...
mod tests {
    ...
    fn detect(value: &str) -> String {
        let detectors = DETECTOR_PRESETS
            ...
            .collect::<Vec<_>>();
        detect_with_detectors(value, &detectors)
    }
    ...
    fn recognizes_structured_text_without_revealing_it() {
        for (value, expected) in [
            ...
            assert_eq!(detect(value), expected, "failed to classify {value}");
        }
    }

    #[test]
    fn recognizes_blocks_tokens_and_prose() {
        assert_eq!(detect("HOST=localhost\nPORT=5432"), "env_block");
        assert_eq!(
            detect("eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.signature"),
            "jwt"
        );
        assert_eq!(
            detect("This is a complete sentence with enough words to be recognized as prose."),
            "prose"
        );
    }
