...
fn extractor_recipe_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
...
fn extractor_recipe_prompt(prompt: &str) -> String {
    format!(
        "Design a fast, deterministic, local Extractor recipe for Pasted. Return only JSON matching the supplied schema.\n\
         ...
         Leave executable and resource paths null when discovery or user setup is required. List concise installation or configuration steps in setupGuidance.\n\
