...
mod tests {
    ...
    fn passphrase_is_salted_and_verified_without_storage_in_plaintext() {
        ...
        configure(&db, "correct horse battery staple").unwrap();
        assert_ne!(db.get_setting(VERIFIER_SETTING).unwrap().unwrap(), encoded);
    }

    #[test]
    fn verifies_existing_argon2_0_5_credentials_without_rewriting_them() {
        let db = db();
        // Generated by Argon2 0.5.3 with its default parameters.
        let encoded = concat!(
            "$argon2id$v=19$m=19456,t=2,p=1$cGFzdGVkLXRlc3Qtc2FsdA$",
            "tUOcj6GrnLgeuTAYVZitP29cUPdgLudtHALn+kTv6k0"
        );
        db.save_setting(VERIFIER_SETTING, encoded).unwrap();
        assert!(verify(&db, "legacy passphrase").unwrap());
        assert!(!verify(&db, "wrong passphrase").unwrap());
        assert_eq!(
            db.get_setting(VERIFIER_SETTING).unwrap().as_deref(),
            Some(encoded)
        );
    }
