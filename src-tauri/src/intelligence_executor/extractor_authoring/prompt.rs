pub(super) fn extractor_recipe_prompt(prompt: &str) -> String {
    format!(
        "Design a fast, deterministic, local Extractor recipe for Pasted. Return only JSON matching the supplied schema.\n\
         ...
         Add {{\"kind\":\"filter_labels_by_confidence\",\"minimumPercent\":80}} to postProcessing when the Extractor returns scored labels, unless the request explicitly asks for a different confidence floor. Do not add it to text-only Extractors. Labels without a provider confidence score remain eligible.\n\
         ...
         Set noOutputExitCodes to nonzero exit codes that mean the command completed normally but found no searchable content. For example, zbarimg uses [4] when it finds no barcode. Use [] when every nonzero code is a failure.\n\
