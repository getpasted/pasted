...
export function MenuSelect({
  ...
  useEffect(() => {
    if (options.length > 0 && !options.some((option) => option.value === value)) onChange(options[0].value);
  ...
  return (
    <>
      <button
        ref={triggerRef}
        type="button"
        className={`menu-select-trigger flex min-w-0 items-center gap-2 rounded-xl border px-2.5 text-left ${className}`}
        aria-label={label}
        aria-haspopup="menu"
      ...
      >
        {leadingIcon}
        <span className="min-w-0 flex-1 truncate py-2 text-xs font-semibold">
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`theme-menu-item flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left disabled:cursor-not-allowed disabled:opacity-40 ${active ? 'is-selected' : ''}`}
                onClick={() => {
