...
const builtin = (id: number, stableRef: string, defaults: typeof appleDefaults | typeof llamaLabelsDefaults, command: string, formats: string[], args?: string[]): MockExtractor => ({
...
export function mockBuiltinExtractors() {
  return [
    ...
    builtin(3, 'extractor:llama-cpp-labels', llamaLabelsDefaults, 'llama-cli', llamaImageFormats,
      ['-hf', 'ggml-org/SmolVLM-500M-Instruct-GGUF', '--offline', '--image', '{input.path}']),
