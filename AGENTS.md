...
- Keep pull request titles and summaries aligned with the actual diff and list the validation that actually ran. Prefer pull requests that address an existing issue, and always link the pull request when a relevant issue exists. A pull request does not require an issue. Use `Closes #123` in the pull request body when the change fully resolves the issue, and `Refs #123` for partial work. Before merging, verify that fully resolved issues use closing references; `Refs` and `See` do not close issues. Do not create, edit, label, close, or reopen GitHub issues unless the user has requested that state change.
